    ...
    return `
      <div class="flex items-center gap-2.5">
        ...
        <div class="min-w-0">
          ...
          <p class="text-[11px] font-mono truncate" style="color:var(--ink-soft);">${time}${routeTxt ? ' · ' + routeTxt : ''}${ev.ubicacion ? ' · ' + ev.ubicacion : ''}${ev.checador?.name ? ' · Checador: ' + ev.checador.name : ''}</p>
        </div>
      </div>
    `;
  }).join('');
